...
func NewUnion(sets ...*Set) (s *Set) {
  ...
  s = &Set{
    items: make(map[uint64]bool, size),
  }
  s.Union(sets...)
  return
}
...
func (s *Set) setItems(m map[uint64]bool) {
  s.items = make(map[uint64]bool, len(m))
  for k, v := range m {
    s.items[k] = v
  }
}
...
func (s *Set) ContainsAny(items ...uint64) bool {
  s.lock.RLock()
  defer s.lock.RUnlock()
	for _, item := range items {
		if _, exists := s.items[item]; exists {
...
func (s *Set) Delete(items ...uint64) *Set {
  s.lock.Lock()
  defer s.lock.Unlock()
	for _, item := range items {
		delete(s.items, item)
...
func (s *Set) Filter(f func(uint64) bool) *Set {
  s.lock.Lock()
  defer s.lock.Unlock()
  for key := range s.items {
    if !f(key) {
...
func (s *Set) FilterItems(f func(uint64) bool) (res []uint64) {
  s.lock.RLock()
  defer s.lock.RUnlock()
  for key := range s.items {
    if f(key) {
