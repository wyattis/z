...
func NewUnion(sets ...*Set) (s *Set) {
  ...
  s = &Set{
    items: make(map[int16]bool, size),
  }
  s.Union(sets...)
  return
}
...
func (s *Set) setItems(m map[int16]bool) {
  s.items = make(map[int16]bool, len(m))
  for k, v := range m {
    s.items[k] = v
  }
}
...
func (s *Set) Contains(items ...int16) bool {
  s.lock.RLock()
...
func (s *Set) Delete(items ...int16) *Set {
  s.lock.Lock()
  defer s.lock.Unlock()
	for _, item := range items {
		delete(s.items, item)
...
func (s *Set) Filter(f func(int16) bool) *Set {
  s.lock.Lock()
  defer s.lock.Unlock()
  for key := range s.items {
    if !f(key) {
...
func (s *Set) FilterItems(f func(int16) bool) (res []int16) {
  s.lock.RLock()
  defer s.lock.RUnlock()
  for key := range s.items {
    if f(key) {
