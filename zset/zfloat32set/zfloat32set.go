...
func NewUnion(sets ...*Set) (s *Set) {
  ...
  s = &Set{
    items: make(map[float32]bool, size),
  }
  s.Union(sets...)
  return
}
...
func (s *Set) setItems(m map[float32]bool) {
  s.items = make(map[float32]bool, len(m))
  for k, v := range m {
    s.items[k] = v
  }
}
...
func (s *Set) Filter(f func(float32) bool) *Set {
  s.lock.Lock()
  defer s.lock.Unlock()
  for key := range s.items {
    if !f(key) {
...
func (s *Set) FilterItems(f func(float32) bool) (res []float32) {
  s.lock.RLock()
  defer s.lock.RUnlock()
  for key := range s.items {
    if f(key) {
