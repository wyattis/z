...
func Resize(s []float64, n int) (res []float64) {
  diff := n - len(s)
  if diff > 0 {
    return append(s, make([]float64, diff)...)
  } else {
    return s[:n]
  }
}
...
func CutAt(val []float64, index int) (left, right []float64) {
  if index < 0 {
    index += len(val)
  }
  if len(val) > index {
...
func Fill (slice []float64, val float64) []float64 {
  for i := range slice {
    slice[i] = val
  }
  return slice
...
// supplied offset
func Remove(slice []float64, offset int, values ...float64) (res []float64) {
...
// same time
func RemoveAt(slice []float64, index int) (res []float64) {
  if index >= len(slice) {
