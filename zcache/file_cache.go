...
func (c *FileCache) onEvict(key interface{}, value interface{}) {
	id := key.(string)
	c.mutex.Lock(id)
...
func (c *FileCache) sync() (err error) {
	oldestKey, _, exists := c.index.GetOldest()
	oldestId := ""
	if exists {
...
func (c *FileCache) slugId(id string) string {
...
func (c *FileCache) create(id string) (file io.WriteCloser, source io.Closer, err error) {
